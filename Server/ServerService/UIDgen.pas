...
uses  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants, System.Classes, Vcl.Graphics,
  ...
  IdBaseComponent, IdComponent, IdRawBase, IdRawClient, IdIcmpClient;
    ...
    function gandon3(r:widestring):string;    //передаем ID для декодирования получаем key
    ...
    function ReadParemS(UIDS,bInfo:string; var countP:integer; var DateL:TdateTime; var CorrectDate:boolean):boolean; //чтение параметров лицензии
    function DateCompare(DateL:Tdatetime):boolean; // сравнение даты лицензии и даты создания продукта
   Const
  wbemFlagForwardOnly = $00000020;
  DateCreate = '31.12.2023'; // если дата лицензии меньше чем текущая то лицензия не канает
  DateLicDefault='28.12.2023'; // дата по умолчанию для некоректной лицензии и отсутствии лицензии
...
try
  ...
  begin
    Log_write('UID','Ошибка чтения реестра :'+E.Message);
    result:=false;
  end;
end;
end;
...
try
  ...
  begin
    Log_write('UID','Ошибка записи в реестр :'+E.Message);
    result:=false;
  end;
end;
end;
...
function gandon3(r:widestring):string;    ////передаем ID для декодирования
var
i,z,n,l,p:integer;
a,b:widestring;
s:widestring;
...
function DateCompare(DateL:Tdatetime):boolean;
begin
  if strtodate(DateCreate)>DateL then
  result:=false
  else result:=true;
end;
...
var
GenID:string;
LenHexGenID,i:integer;
Decrypt,Bkey,aKey:string;
posStart,PosEnd:integer;
TMPcount:integer;
begin
  try

      aKey:=gandon3(UIDS);// код активации
      GenID:='';
      TMPcount:=10;
      DateL:=strtodate(DateLicDefault);

      Bkey:=StringReplace(aKey,'-','',[rfReplaceAll]); // формируем из кода активации 1 код для шифрования
      LenHexGenID:=length(bInfo); // длинна строки
      for I := 1 to LenHexGenID do
      begin
        if i<>LenHexGenID then
        if i mod 7 = 0 then continue;   // каждый 7й символ дефис-, пропускаем его
        GenID:=GenID+bInfo[i];
      end;
      GenID:=HexTostr(GenID);
     if DeCryptstrs(GenID,BKey,Decrypt) then {/500/28.12.2023} // если расшифровали строку
      Begin
        Delete(Decrypt, 1, 1);
        posEnd:=pos('/',Decrypt);

        if not TryStrToInt(copy(Decrypt,1,posEnd-1),TMPcount) then // определяем кол-во ПК
        begin
         TMPcount:=10;
         DateL:=strtodate(DateLicDefault);
        end;

        Delete(Decrypt,1,posEnd);
          if not TryStrToDate(Decrypt,DateL) then // определяем дату лицензии
          begin
           result:=false;
           CorrectDate:=false; // признак некорректной даты
           CountP:=10;         // кол-во пк
           DateL:=strtodate(DateLicDefault);  // установка даты по умолчанию
          end
          else
          begin
            if DateCompare(DateL) then // если дата лицензии актуальна
            begin
             CorrectDate:=true; // дата актуальна для данной версии
             result:=true;     // результат все ок
             CountP:=TMPcount;   // установка кол-ва пк
            end
            else
            begin
             CorrectDate:=false; // признак некорректной даты
             DateL:=strtodate(DateLicDefault);  // установка даты по умолчанию
             CountP:=10;      // кол-во пк
             result:=false;    // результат отрицательный
            end;
          end;
      End
      else
